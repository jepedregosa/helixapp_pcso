function voucherApprovalList(){
	$$.ajax({
		url: SERVER_ADDRESS + "/tranlist?module=voucher&username="+sessionStorage.getItem('username'),
		contentType: 'jsonp',
		method: 'POST',
		type: 'POST',
		dataType : 'jsonp',
		...
		success: function( response ) {
			...
			mainView.router.loadPage({url:'./modules/voucher/approval/voucher.html', ignoreCache:true,
				context:{
...
function voucherdetailApprovalList(obj){
	$$.ajax({
		url: SERVER_ADDRESS + "/tranlist?module=voucher&username="+sessionStorage.getItem('username')+"&BatNbr="+obj.title,
		contentType: 'jsonp',
		method: 'POST',
		type: 'POST',
		dataType : 'jsonp',
		...
		success: function( response ) {
			$$.ajax({
				url: SERVER_ADDRESS + "/tranlist?module=voucher&option=detail&BatNbr="+obj.title,
				contentType: 'jsonp',
				method: 'POST',
				type: 'POST',
				dataType : 'jsonp',
				...
				success: function( response2 ) {
					mainView.router.loadPage({url:'./modules/voucher/approval/voucherdetail.html', ignoreCache:true,
					context:{
						forapproval: JSON.parse(response),
...
myApp.onPageAfterAnimation('voucherApp', function (page) {
	myApp.showIndicator();
