function charitylist(){
	$$.ajax({
		url: SERVER_ADDRESS + "/tranlist?module=charity&username="+sessionStorage.getItem('username'),
		contentType: 'jsonp',
		method: 'POST',
		type: 'POST',
		dataType : 'jsonp',
		...
		success: function( response ) {
			...
			mainView.router.loadPage({url:'./modules/charity/charity.html', ignoreCache:true,
				context:{
...
function charitydetaillist(obj){
	$$.ajax({
		...
		success: function( response ) {
			$$.ajax({
				...
				success: function( response2 ) {
					mainView.router.loadPage({url:'./modules/charity/charitydetail.html', ignoreCache:true,
					context:{
						forapproval: JSON.parse(response),
...
myApp.onPageAfterAnimation('charity', function (page) {
	myApp.showIndicator();
