function prizeclaimApprovalList(){
	mainView.router.loadPage({url:'./modules/prizeclaim/approval/prizeclaim.html', ignoreCache:true});	
}

function prizeclaimdetailApprovalList(obj){
	$$.ajax({
		url: SERVER_ADDRESS + "/tranlist?module=prizeclaim&username="+sessionStorage.getItem('username')+"&BatNbr="+obj.title,
		contentType: 'jsonp',
		method: 'POST',
		type: 'POST',
		dataType : 'jsonp',
		...
		success: function( response ) {
			$$.ajax({
				url: SERVER_ADDRESS + "/tranlist?module=prizeclaim&option=detail&BatNbr="+obj.title,
				contentType: 'jsonp',
				method: 'POST',
				type: 'POST',
				dataType : 'jsonp',
				...
				success: function( response2 ) {
					mainView.router.loadPage({url:'./modules/prizeclaim/approval/prizeclaimdetail.html', ignoreCache:true,
					context:{
...
myApp.onPageAfterAnimation('prizeclaimApp', function (page) {
	myApp.showIndicator();
	
	$$.ajax({
		url: SERVER_ADDRESS + "/tranlist?module=prizeclaim&username="+sessionStorage.getItem('username'),
		contentType: 'jsonp',
		method: 'POST',
		type: 'POST',
		dataType : 'jsonp',
