/* ===== Virtual List ===== */
myApp.onPageInit('goodsreceiptApp', function (page) {
 	myApp.showIndicator();
...
function goodsreceiptApprovalList(){
	mainView.router.loadPage({url:'./modules/goodsreceipt/approval/goodsreceipt.html', ignoreCache:true});
}

function goodsreceiptdetailApprovalList(obj){
	$$.ajax({
		...
		success: function( response ) {
			$$.ajax({
				...
				success: function( response2 ) {
					mainView.router.loadPage({url:'./modules/goodsreceipt/approval/goodsreceiptdetail.html', ignoreCache:true,
					context:{
